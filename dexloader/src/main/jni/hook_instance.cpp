...
/*
int readlink(const char* linkstr, char* linkpath, int length) {
    std::string result;
    if (!android::base::Readlink(linkstr, &result)) {
        throwErrnoException(env, "readlink");
        return -1;
    }
    char* resultstr = result.c_str();
    int i = 0;
    while(resultstr[i]&&i<length) {
        linkpath[i] = resultstr[i];
        i++;
    }
    return i;
}*/
