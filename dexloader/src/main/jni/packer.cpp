...
unsigned char MINIDEX[292] = {
        ,
        ...
        
};
...
void write_mix_dex(const char *minidex) {
    ...
    if (access(minidex, F_OK) == -1) {
        LOGI("[+]write_mix_dex minidex:%s", minidex);
...
jint mem_loadDex_dvm(JNIEnv *env, char *szPath) {
    ...
    LOGI("mem_loadDex_dvm dlsym dvm_dalvik_system_DexFile");
    ...
    LOGI("mem_loadDex_dvm munmap");

    u4 args[] = {(u4) (long) ao};
    union JValue pResult;
    jint result;
    if (openDexFile != NULL) {
        LOGI("mem_loadDex_dvm openDexFile != NULL");
...
char *get_path_frommaps(const char *pkgName, char *filter1, char *filter2) {
    ...
    while (true) {
        ...
        if (!fgets(line_buffer, 255, fd))
            break;
        //寻找带有包名和.dex或.odex的行
        if (strstr(line_buffer, pkgName) &&
            (strstr(line_buffer, filter1) || strstr(line_buffer, filter2))) {
...
jobject load_dex_fromfile(JNIEnv *env, const char *inPath, const char *outPath) {
    ...
    LOGI("[+]load_dex_fromfile start ExceptionCheck");
    if (env->ExceptionCheck()) {
        LOGE("[-]get loadDex methodID  error");
        return 0;
    }
    LOGI("[+]load_dex_fromfile ExceptionCheck succeed");
...
void* getLibartHandler(const char* libartName)
{
    ...
    if (g_sdk_int < 23) {
        ...
        void *jit_lib = fake_dlopen(LIB_ART_COMPILER_PATH, RTLD_NOW);
        LOGI("fake dlopen install: %p", handle_art);
    }
    if (!handle_art) {
        LOGE("[-]get %s handle failed:%s", dlerror());
        return NULL;
...
void mem_loadDex(JNIEnv *env, jobject ctx, const char *szDexPath) {
    ...
    sprintf(inPath, "%s/.shell/mini.dex", g_file_dir);
...
void native_attachBaseContext(JNIEnv *env, jobject obj, jobject ctx) {
    ...
    jfieldID nativeLibraryDir_field = env->GetFieldID(ApplicationInfoClass, "nativeLibraryDir",
                                                      "Ljava/lang/String;");
    jstring nativeLibraryDir = static_cast<jstring>(env->GetObjectField(ApplicationInfo_obj,
    ...
    jmethodID getPackageName = env->GetMethodID(ApplicationClass, "getPackageName",
                                                "()Ljava/lang/String;");
    ...
    char shellPath[256] = {0};//加密dex的存储路径
    char shellDir[256] = {0}; //加密dex的所在目录
    sprintf(shellDir, "%s/.shell", g_file_dir);
    sprintf(shellPath, "%s/.shell/%s", g_file_dir, JIAMI_MAGIC);
    LOGD("[+]shellDir:%s,shellPath:%s", shellDir, shellPath);

    //extract encrypted dex from assets
    extract_file(env, ctx, shellDir, shellPath, JIAMI_MAGIC);
    //mem loadDex
