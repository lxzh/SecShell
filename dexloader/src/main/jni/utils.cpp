...
unsigned int search_symbol_fromelf(char *lib_path, unsigned int lib_base_addr, char *target) {
    ...
    lseek(fd, 0, SEEK_SET);
    int buffer = (int) (long) mmap(0, size, 1, 2, fd, 0);
    if (!buffer) {
        close(fd);
        return 0;
    }
...
#define DT_STRSZ    10
    ...
    do {
        if (*(int *) ptr == PT_DYNAMIC) {
            ...
            LOGD("[+]pt_dynamic_vaddr:%08x,lib_base_addr:%08x,pt_load_based:%08x,ynamic_addr:%08x",
                 pt_dynamic_vaddr, lib_base_addr, pt_load_base, dynamic_addr);
            if (dynamic_addr) {
                ...
                while (memcmp((char *) (strtab_addr + *(int *) (p_sym)), target, strlen(target)) !=
                       0) {
...
void *get_addr_symbol(char *module_name, char *target_symbol) {
    ...
    void *result = (void *) search_symbol_fromelf(module_name,
                                                  (unsigned int) (unsigned long) module_base,
                                                  target_symbol);
    if (!result) {
...
int extract_file(JNIEnv *env, jobject ctx, const char *dir, const char *szDexPath,
                 const char *fileName) {
